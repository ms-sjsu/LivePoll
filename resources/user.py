...
class EmailInvite(Resource):
    ...
    def post(self):
        ...
        print(data)
        try:
            query = 'insert into eba_quiz_invites values (%s,%s,%s,%s,%s,%s,%s,%s,%s)'
            cursor.execute(query, (str(uuid.uuid4()),data['client_email_id'],data['tenancy_id'],data['qset_id'],'open','yes','1',data['created_by'],datetime.datetime.now(),))
            db.commit()
        except:
            logger.error('Error : Unexpected error: Error during the user registeration!')
            return("message", "Unable to Register user! Erro Occurred"), 400
    ...
    def get(self):
        ...
        cursor = db.cursor()
        if data['querytype'] == 'recruiter':
            query = "select invite_id,client_email_id,qset_id,email_sent, created_by from eba_quiz_invites where upper(created_by)=%s and tenancy_id=%s"
            argument1 = data['created_by'].upper()
            argument2 = data['tenancy_id']
            print(argument1)
            args = (argument1, argument2)
            ...
            query = "select invite_id,client_email_id,qset_id,email_sent, created_by from eba_quiz_invites where tenancy_id=%s"
            argument = data['tenancy_id']
            cursor.execute(query,argument)
        else:
           query = "select invite_id,client_email_id,qset_id,email_sent, created_by from eba_quiz_invites where 1=2"
           cursor.execute(query)
        row_headers = [x[0] for x in cursor.description]
        userdata = cursor.fetchall()
        json_data=[]
